...
If the measured area equals to the box area, then it can be said that the output has no dependence on that input (as occurs for $x_2$ in Figure X).
...
%Show and explain figures with the sampling method

Figure 2 shows the 20 subintervals computed with interval arithmetic using the IntervalArithmetic package in Julia [REF].
Note that since the function $y = f(x_1,x_2)$ is a black-box function, technically it should not be feasible to have access to
its analytical description, and therefore the interval arithmetic approach would not be possible.
Therefore, it has been included as an example to show how the interval arithmetic approach would work.
...
Note that the interval arithmetic approach captures that $x_2$ has no impact on the uncertainty of $y$ (it also can be seen in Figure 2, since the
grey area equals to the red area), whilst the sampling approach is not that precise.
...
\begin{figure}
	...
	\begin{tabular}{@{}c@{}}
	  ...
	  \small (a) Visualisation of the 20 subintervals of $y=f(x_1,x_2)$ and $x_1$.
	\end{tabular}
	...
	\begin{tabular}{@{}c@{}}
	  ...
	  \small (b) Visualisation of the 20 subintervals of $y=f(x_1,x_2)$ and $x_2$.
	\end{tabular}

	\caption{Results of the evaluation of $y=f(x_1,x_2)$ with $x_1,x_2 \in [-5,-5]$ using 20 subintervals and interval arithmetic.
	The sensitivity index is calculated as in Equation 1, where the numerator is equal to the area coloured in grey and the denominator
	equal to the area coloured in red.}
	%Function $y$ shows strong dependence with $x_1$, but little or no dependence with $x_2$.}
	\label{fig:myfig}
\end{figure}
